# Notice — ЧАРО Messenger

## Copyright

Copyright © 2024-2026 Бутаев Алексей Юрьевич (Charo Team). All rights reserved.

ЧАРО (Charo) Messenger — proprietary software. All source code, assets,
designs, trademarks, and documentation are the exclusive property of Бутаев Алексей Юрьевич (Charo Team).

## Trademarks

- **ЧАРО** and **Charo** are registered trademarks of Бутаев Алексей Юрьевич (Charo Team).
- The Charo logo and app icon are proprietary assets.
- No third-party may use these marks without written permission.

## Third-Party Licenses

This software uses open-source components:

- Flutter (BSD-3-Clause, Google LLC)
- Fastify (MIT, Matteo Collina & Coda Hale)
- Prisma (Apache-2.0, Prisma Data Platform GmbH)
- Zod (MIT, Colin McDonnell)
- ioredis (MIT, Zihua Li)
- jsonwebtoken (MIT, Auth0)
- bcryptjs (BSD-3-Clause, Daniel Wirtz)
- SQLite3 / Drift (various, see pubspec.yaml)
- Signal Protocol (GPLv3, Signal Messenger) — used via libsignal_protocol_dart

Full license texts available in `node_modules/` and `.pub-cache/`.

## Data Processing

Personal data processing notice per ФЗ-152, GDPR, CCPA:
- See [PRIVACY_POLICY.md](./PRIVACY_POLICY.md)

## Contact

- Legal: pizdec666@yandex.ru
- Security: pizdec666@yandex.ru
- General: pizdec666@yandex.ru
